...
def test_return_edges_handles_regular_non_oneway_paths(full_fat_default_config):
    ...
    edges = osmnx_customised.return_edges(paths, config=full_fat_default_config, bidirectional=False)
    assert len(edges) == 2
    assert (0, 1) in [edge for edge, attribs in edges]
    assert (1, 0) in [edge for edge, attribs in edges]
    for edge, attribs in edges:
        assert_semantically_equal(attribs,
                                  {'modes': ['bike', 'car', 'walk'], 'osmid': 0,
...
def test_return_edges_handles_oneway_paths(full_fat_default_config):
    ...
    for edge, attribs in edges:
        assert_semantically_equal(
            attribs,
            {'modes': ['bike', 'car', 'walk'], 'osmid': 0, 'highway': 'unclassified', 'oneway': 'yes'})
...
def test_return_edges_handles_reversed_oneway_paths(full_fat_default_config):
    ...
    assert (1, 0) in [edge for edge, attribs in edges]
    for edge, attribs in edges:
        assert_semantically_equal(attribs, {'modes': ['bike', 'car', 'walk'], 'osmid': 0, 'highway': 'unclassified', 'oneway': 'reverse'})
